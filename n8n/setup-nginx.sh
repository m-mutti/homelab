...
if [ -f .env ]; then
    ...
    echo "Error: .env file not found. Run this script from the n8n directory."
    exit 1
fi
...
cat > "${SCRIPT_DIR}/n8n.generated.conf" <<EOF
server {
    listen 80;
    server_name ${DOMAIN};
...
sudo ln -sf "${SCRIPT_DIR}/n8n.generated.conf" "${SITES_AVAILABLE}/n8n"
sudo ln -sf "${SITES_AVAILABLE}/n8n" "${SITES_ENABLED}/n8n"
...
envsubst '${DOMAIN} ${N8N_PORT}' < "${SCRIPT_DIR}/n8n.nginx.conf" > "${SCRIPT_DIR}/n8n.generated.conf"

# Test and reload nginx with full config
sudo nginx -t
sudo systemctl reload nginx

echo "Done! n8n is available at https://${DOMAIN}"
