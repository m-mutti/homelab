...
if [ -f .env ]; then
    ...
    echo "Error: .env file not found. Run this script from the homarr directory."
    exit 1
fi
...
cat > "${SCRIPT_DIR}/homarr.generated.conf" <<EOF
server {
    listen 80;
    server_name ${DOMAIN};
...
sudo ln -sf "${SCRIPT_DIR}/homarr.generated.conf" "${SITES_AVAILABLE}/homarr"
sudo ln -sf "${SITES_AVAILABLE}/homarr" "${SITES_ENABLED}/homarr"
...
envsubst '${DOMAIN} ${HOMARR_PORT}' < "${SCRIPT_DIR}/homarr.nginx.conf" > "${SCRIPT_DIR}/homarr.generated.conf"

# Test and reload nginx with full config
sudo nginx -t
sudo systemctl reload nginx

echo "Done! Homarr is available at https://${DOMAIN}"
